...
// the private key contains the public key and the components secret to the private key
...
func newEC2SigningKeyWithPrivateKey(privateKey *ecdsa.PrivateKey) *ec2SigningKey {
	info := ec2KeyInfo{
		...
		Curve:     ec2P256Curve,
		X:         privateKey.X.Bytes(), // X of the public key
		Y:         privateKey.Y.Bytes(), // Y of the public key
	}
